...
public final class JsonSchemaReader {
    ...
    public static @Nonnull JsonSchemaObject readFromFile(@Nonnull Project project, @Nonnull VirtualFile file) throws Exception {
        ...
        if (object == null) {
            throw new Exception(JsonLocalize.schemaReaderCantLoadModel(file.getName()).get());
        }
        return object;
    }

    public static @Nullable String checkIfValidJsonSchema(@Nonnull Project project, @Nonnull VirtualFile file) {
        final long length = file.getLength();
        final String fileName = file.getName();
        if (length > MAX_SCHEMA_LENGTH) {
            return JsonLocalize.schemaReaderFileTooLarge(fileName, length).get();
        }
        if (length == 0) {
            return JsonLocalize.schemaReaderFileEmpty(fileName).get();
        }
        try {
            readFromFile(project, file);
        }
        catch (Exception e) {
            final String message = JsonLocalize.schemaReaderFileNotFoundOrError(fileName, e.getMessage()).get();
    ...
    private @Nonnull JsonSchemaObjectImpl read(@Nonnull JsonValueAdapter rootAdapter) {
        ...
        while (!myQueue.isEmpty()) {
            ...
            if (currentSchema.getId() != null) {
                myIds.put(currentSchema.getId(), currentSchema);
            }
    ...
    private void readSingleDefinition(@Nonnull String name,
        ...
        if (definitions == null) {
            schema.setDefinitionsMap(definitions = new HashMap<>());
        }
    ...
    private static void fillMap() {
        ...
        READERS_MAP.put(X_INTELLIJ_CASE_INSENSITIVE, (element, object, queue, virtualFile) -> {
            if (element.isBooleanLiteral()) {
                object.setForceCaseInsensitive(getBoolean(element));
            }
        });
        ...
        READERS_MAP.put("exclusiveMaximum", (element, object, queue, virtualFile) -> {
            if (element.isBooleanLiteral()) {
                ...
                object.setExclusiveMaximumNumber(getNumber(element));
            }
        });
        READERS_MAP.put("exclusiveMinimum", (element, object, queue, virtualFile) -> {
            if (element.isBooleanLiteral()) {
                ...
                object.setExclusiveMinimumNumber(getNumber(element));
            }
        });
        ...
        READERS_MAP.put("uniqueItems", (element, object, queue, virtualFile) -> {
            if (element.isBooleanLiteral()) {
                object.setUniqueItems(getBoolean(element));
            }
        });
        ...
        return (element, object, queue, virtualFile) -> {
            if (element instanceof JsonArrayValueAdapter) {
                ...
                for (JsonValueAdapter value : list) {
                    ...
                    if (enumValue == null) {
                        return; // don't validate if we have unsupported entity kinds
                    }
                ...
                for (JsonPropertyAdapter property : list) {
                    ...
                    if (value instanceof JsonArrayValueAdapter) {
                        ...
                        if (!dependencies.isEmpty()) {
                            propertyDependencies.put(property.getName(), dependencies);//fixme????
                        }
                    }
    ...
    private static @Nonnull Map<String, Object> readExample(@Nonnull JsonValueAdapter element) {
        ...
        if (!(element instanceof JsonObjectValueAdapter objectAdapter)) {
            return example;
        }
