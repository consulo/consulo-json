...
import com.jetbrains.jsonSchema.JsonSchemaCatalogManager;
...
public final class JsonFileResolver {
    ...
    public static @Nullable VirtualFile resolveSchemaByReference(@Nullable VirtualFile currentFile,
        ...
        if (schemaUrl == null || StringUtil.isEmpty(schemaUrl)) {
            return null;
        }
        ...
        else if (!schemaUrl.startsWith("http") || !JsonRegistry.JSON_SCHEME_OBJECT_V2) {
    ...
    private static final LoadingCache<String, VirtualFile> urlValidityCache =
        CacheBuilder.newBuilder()
            ...
            .maximumSize(1000)
            .build(CacheLoader.from(JsonFileResolver::computeVirtualFileForValidUrlOrNull));
    ...
    public static boolean isAbsoluteUrl(@Nonnull String path) {
        return isHttpPath(path) ||
            ...
            FileUtil.toSystemIndependentName(path).startsWith(StandardFileSystems.JAR_PROTOCOL_PREFIX);
    }
