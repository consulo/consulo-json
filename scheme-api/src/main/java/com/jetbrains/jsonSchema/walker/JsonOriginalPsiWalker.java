...
import com.jetbrains.jsonSchema.internal.adapter.JsonJsonPropertyAdapter;
...
public class JsonOriginalPsiWalker implements JsonLikePsiWalker {
    ...
    public @Nonnull Collection<PsiElement> getRoots(@Nonnull PsiFile file) {
        return file instanceof JsonFile ? ContainerUtil.createMaybeSingletonList(((JsonFile) file).getTopLevelValue()) : List.of();
    }
    ...
    private static class JsonOriginalSyntaxAdapter implements JsonLikeSyntaxAdapter {
        ...
        @Nonnull
        @Override
        public PsiElement createEmptyArray(@Nonnull Project project) {
            return new JsonElementGenerator(project).createEmptyArray();
        }
        ...
        public void removeArrayItem(@Nonnull PsiElement item) {
            ...
            if (prev instanceof LeafPsiElement && ((consulo.language.impl.psi.LeafPsiElement) prev).getElementType() == JsonElementTypes.COMMA) {
                prev.delete();
            }
            else if (next instanceof LeafPsiElement && ((consulo.language.impl.psi.LeafPsiElement) next).getElementType() == JsonElementTypes.COMMA) {
                next.delete();
            }
            item.delete();
        }
        ...
        public void removeIfComma(PsiElement forward) {
            if (forward instanceof consulo.language.impl.psi.LeafPsiElement leaf) {
                if (leaf.getElementType() == JsonElementTypes.COMMA) {
                    forward.delete();
                }
                if (leaf.getElementType() == JsonElementTypes.R_CURLY && PsiTreeUtil.skipWhitespacesBackward(leaf) instanceof consulo.language.impl.psi.LeafPsiElement prev &&
