...
import com.jetbrains.jsonSchema.internal.*;
...
public final class JsonSchemaCatalogManager {
    ...
    public static final String DEFAULT_CATALOG = "http://schemastore.org/api/json/catalog.json";
    public static final String DEFAULT_CATALOG_HTTPS = "https://schemastore.org/api/json/catalog.json";
    ...
    private final Map<Runnable, FileDownloadingAdapter> myDownloadingAdapters = Maps.newConcurrentWeakHashMap();
    ...
    public void startUpdates() {
        JsonSchemaCatalogProjectConfiguration.getInstance(myProject).addChangeHandler(() -> {
            update();
            JsonSchemaService.get(myProject).reset();
        });
    ...
    public void unregisterCatalogUpdateCallback(@Nonnull Runnable callback) {
        if (!myDownloadingAdapters.containsKey(callback)) {
            return;
        }
    ...
    private static @Nullable String getRelativePath(@Nonnull VirtualFile file, @Nonnull Project project) {
        ...
        if (basePath != null) {
            basePath = StringUtil.trimEnd(basePath, VirtualFileUtil.VFS_SEPARATOR_CHAR) + VirtualFileUtil.VFS_SEPARATOR_CHAR;
        ...
        return contentRoot != null ? VirtualFileUtil.findRelativePath(contentRoot, file, VirtualFileUtil.VFS_SEPARATOR_CHAR) : null;
    }
